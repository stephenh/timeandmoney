...
public class DurationTest extends TestCase {
  ...
  public void testAddMillisecondsToPoint() {
    ...
    Assert.assertEquals(dec22At1, twoDays.addedTo(dec20At1));
  }
  ...
  public void testSubtractMillisecondsFromPoint() {
    ...
    Duration twoDays = Duration.days(2);
    Assert.assertEquals(dec18At1, twoDays.subtractedFrom(dec20At1));
  }
  ...
  public void testSubtractMonthsFromPoint() {
    ...
    TimePoint dec20At1 = TimePoint.atGMT(2003, 12, 20, 01, 0, 0, 0);
    Duration twoMonths = Duration.months(2);
    Assert.assertEquals(oct20At1, twoMonths.subtractedFrom(dec20At1));
  ...
  public void testSubtractMonthsFromCalendarDate() {
    ...
    Duration twoMonths = Duration.months(2);
    Assert.assertEquals(oct20, twoMonths.subtractedFrom(dec20));

    Duration sixtyoneDays = Duration.days(61);
    Assert.assertEquals(oct20, sixtyoneDays.subtractedFrom(dec20));
  ...
  public void testAddToCalendarDate() {
    ...
    Duration twoMonths = Duration.months(2);
    Assert.assertEquals(dec20_2003, twoMonths.addedTo(oct20_2003));

    Duration sixtyoneDays = Duration.days(61);
    Assert.assertEquals(dec20_2003, sixtyoneDays.addedTo(oct20_2003));

    CalendarDate dec20_2001 = CalendarDate.from(2001, 12, 20);
    Duration twoYears = Duration.years(2);
    Assert.assertEquals(dec20_2003, twoYears.addedTo(dec20_2001));
  }

  public void testConversionToBaseUnits() {
    Duration twoSeconds = Duration.seconds(2);
    Assert.assertEquals(2000, twoSeconds.inBaseUnits());
  }

  public void testEquals() {
    Assert.assertEquals(Duration.days(2), Duration.hours(48));
    Assert.assertEquals(Duration.years(1), Duration.quarters(4));
  }

  public void testAdd() {
    Assert.assertEquals(Duration.days(2), Duration.hours(24).plus(Duration.days(1)));
    Assert.assertEquals(Duration.months(4), Duration.months(1).plus(Duration.quarters(1)));
  }

  public void testSubtract() {
    Assert.assertEquals(Duration.days(2), Duration.days(3).minus(Duration.hours(24)));
    Assert.assertEquals(Duration.months(2), Duration.quarters(1).minus(Duration.months(1)));
  }

  public void testDivide() {
    Assert.assertEquals(new BigDecimal(1.5), Duration.days(3).dividedBy(Duration.days(2)).decimalValue(1, Rounding.DOWN));
  }

  public void testToNormalizedString() {
    Assert.assertEquals("2 days", Duration.days(2).toNormalizedString());
    Duration complicatedDuration = Duration.daysHoursMinutesSecondsMilliseconds(5, 4, 3, 2, 1);
    Assert.assertEquals("5 days, 4 hours, 3 minutes, 2 seconds, 1 millisecond", complicatedDuration.toNormalizedString());
    Assert.assertEquals("52 weeks, 1 day", Duration.days(365).toNormalizedString());
  }

  public void testToNormalizedStringMonthBased() {
    Assert.assertEquals("2 months", Duration.months(2).toNormalizedString());
    Assert.assertEquals("1 year, 1 quarter, 1 month", Duration.months(16).toNormalizedString());
  }

  public void testToString() {
    Assert.assertEquals("21 days", Duration.weeks(3).toString()); //Weeks are not conventional to read.
    Assert.assertEquals("1 year, 4 months", Duration.months(16).toString()); //Quarters are not conventionalto read.
  }
  ...
  public void testCompare() {
    ...
    Assert.assertTrue(oneHour.compareTo(twoHours) < 0);
    Assert.assertTrue(oneHour.compareTo(sixtyMinutes) == 0);
    Assert.assertTrue(twoHours.compareTo(oneHour) > 0);
  }
  ...
  public void testStartingFromTimePoint() {
    ...
    Assert.assertEquals(dec20_1_3, Duration.hours(2).startingFrom(dec20At1));
  }
  ...
  public void testStartingFromCalendarDate() {
    ...
    Assert.assertEquals(dec20_26, Duration.days(7).startingFrom(dec20));
  }

  public void testNormalizedUnit() {
    Assert.assertEquals(TimeUnit.second, Duration.seconds(30).normalizedUnit());
    Assert.assertEquals(TimeUnit.minute, Duration.seconds(120).normalizedUnit());
    Assert.assertEquals(TimeUnit.day, Duration.hours(24).normalizedUnit());
    Assert.assertEquals(TimeUnit.hour, Duration.hours(25).normalizedUnit());
  }
